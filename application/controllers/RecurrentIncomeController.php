<?php
class RecurrentIncomeController extends Zend_Controller_Action{
	...
	{		
		...
		$select = $expenses->getMapper()->getDbTable()->select()
				->from(array('p'=>'recurrent_expenses'),
				...
				if(!empty($params['idGroupSearch'])){
					...
					$select->where('p.date <= ?',$params['toDate']);
                                ...
                                $select->joinLeft(array('uf' => 'uploaded_files'), 'p.`id` = uf.`idMessage`',array('ufiles'=>'uf.id', 'recurrent'=>'uf.idUser'))
                                        //recurrent uploaded files have idUser =1
                                        ->setIntegrityCheck(false);
                                $select->order(array('date DESC'));
                
                $resultIncome = Needs_Tools::showRecurrentIncomeDashboardbyDate((!empty($params['fromDate'])?$params['fromDate']:date('Y-m-01')), (!empty($params['toDate'])?$params['toDate']:date('Y-m-d')));
                $this->view->resultIncome=$resultIncome;
                
		//END:SELECT PROJECTS
		$form = new Default_Form_RecurrentIncome();	
		$form->setDecorators(array('ViewScript', array('ViewScript', array('viewScript' => 'forms/recurrent-income/add-income.phtml'))));
		$this->view->form = $form;
                
		if($this->getRequest()->isPost() && $this->getRequest()->getParam('control')=='addIncome')
		{
			if($form->isValid($this->getRequest()->getPost())) 
			{
				...
				if($expenseId = $model->save())
				{
					...
					//mesaj de succes
					$this->_flashMessenger->addMessage("<div class='success  canhide'><p>Recurrent income was added successfully<a href='javascript:;'></a><p></div>");
				}else{
					//mesaj de eroare
					$this->_flashMessenger->addMessage("<div class='failure canhide'><p>Recurrent income was not added<a href='javascript:;'></a><p></div>");
				}
				//redirect
				$this->_redirect(WEBROOT.'recurrent-income');	
			}	
		}
		
		$formsearch = new Default_Form_RecurrentIncomeSearch();
		$formsearch->setDecorators(array('ViewScript', array('ViewScript', array('viewScript' => 'forms/recurrent-income/income-search.phtml'))));
		...
		if($id)
		{
			...
			if($this->getRequest()->isPost())
			{
				...
				if($form_comments->isValid($this->getRequest()->getPost())) 
				{
					...
					$this->_redirect(WEBROOT.'/recurrent-income/details/id/'.$id);
				}
			}
...
//			}
	...
	public function editAction()
	{
		...
		if ($model->find($id)) {
			...
			$form->edit($model);
				$form->setDecorators(array('ViewScript', array('ViewScript', array('viewScript' => 'forms/recurrent-income/edit-income.phtml'))));
			$this->view->form = $form;
				...
				if ($this->getRequest()->getPost('submit')) {
                                                    ...
                                                    if(!empty($post['galleryFiles']) && is_array($post['galleryFiles']))
					{
						...
						//END:SAVE ATTACHMENTS
							...
							$post = $this->getRequest()->getPost();
							//mesaj de succes
							$this->_flashMessenger->addMessage("<div class='success  canhide'><p>Recurrent income was modified successfully<a href='javascript:;'></a></p></div>");
						} else {
							$this->_flashMessenger->addMessage("<div class='failure canhide'><p>Recurrent income was not modified<a href='javascript:;'></a></p></div>");
						}
	...
	public function deleteAction() {
		...
		if ($model->find($id)) {
			if ($expenseId = $model->delete()) {
					//Needs_Tools::DeleteLegaturi( $expenseId );
					$this->_flashMessenger->addMessage("<div class='success canhide'><p>Recurrent income was deleted successfully!<a href='javascript:;'></a></p></div>");
				} else {
					$this->_flashMessenger->addMessage("<div class='failure canhide'><p>Recurrent income was not deleted!<a href='javascript:;'></a></p></div>");
				}
			$this->_redirect('recurrent-income');
		}
	}
}