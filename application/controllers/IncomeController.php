...
class IncomeController extends Zend_Controller_Action{
	...
	{		
		...
		$select->order(array('date DESC'));
                ...
                $resultIncome = Needs_Tools::showIncomeDashboardbyDate((!empty($params['fromDate'])?$params['fromDate']:date('Y-m-01')), (!empty($params['toDate'])?$params['toDate']:date('Y-m-d')), (!empty($params['idGroupSearch'])?$params['idGroupSearch']:''));
                $this->view->resultIncome = $resultIncome;

		//END:SELECT INCOME
		$form = new Default_Form_Income();
		$form->setDecorators(array('ViewScript', array('ViewScript', array('viewScript' => 'forms/income/add-income.phtml'))));
		$this->view->form = $form;
				...
				if($expenseId = $model->save())
				{
                                    if(!empty($post['galleryFiles']) && is_array($post['galleryFiles']))
					{
						...
						if($tmpFiles->find($valuesGallery)){
                                                    ...
                                                    $gallery->setIdUser(0);//0 - means the image is for non recurrent income
					...
					//mesaj de succes
					$this->_flashMessenger->addMessage("<div class='success  canhide'><p>Income was added successfully<a href='javascript:;'></a><p></div>");
				}else{
					//mesaj de eroare
					$this->_flashMessenger->addMessage("<div class='failure canhide'><p>Income was not added<a href='javascript:;'></a><p></div>");
				}
				//redirect
				$this->_redirect(WEBROOT.'income');	
			}	
		}
		
		$formsearch = new Default_Form_IncomeSearch();
		$formsearch->setDecorators(array('ViewScript', array('ViewScript', array('viewScript' => 'forms/income/income-search.phtml'))));
		...
		if($id)
		{
			...
			if($this->getRequest()->isPost())
			{
				...
				if($form_comments->isValid($this->getRequest()->getPost())) 
				{
					...
					$this->_redirect(WEBROOT.'/income/details/id/'.$id);
				}
			}
...
//			}
	...
	public function editAction()
	{
		...
		if ($model->find($id)) {
			...
			$form->edit($model);
				$form->setDecorators(array('ViewScript', array('ViewScript', array('viewScript' => 'forms/income/edit-income.phtml'))));
			$this->view->form = $form;
				...
				if ($this->getRequest()->getPost('submit')) {
                                                    ...
                                                    if(!empty($post['galleryFiles']) && is_array($post['galleryFiles']))
					{
						...
						//END:SAVE ATTACHMENTS
							...
							$post = $this->getRequest()->getPost();
							//mesaj de succes
							$this->_flashMessenger->addMessage("<div class='success  canhide'><p>Income was modified successfully<a href='javascript:;'></a></p></div>");
						} else {
							$this->_flashMessenger->addMessage("<div class='failure canhide'><p>Income was not modified<a href='javascript:;'></a></p></div>");
						}
	...
	public function deleteAction() {
		...
		if ($model->find($id)) {
			if ($expenseId = $model->delete()) {
					Needs_Tools::DeleteLegaturi( $expenseId );
					$this->_flashMessenger->addMessage("<div class='success  canhide'><p>Income was deleted successfully!<a href='javascript:;'></a></p></div>");
				} else {
					$this->_flashMessenger->addMessage("<div class='failure canhide'><p>Income was not deleted!<a href='javascript:;'></a></p></div>");
				}
			$this->_redirect('income');
		}
	}
}
